...
class BasicBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, stride=1):
        ...
        self.frelu1 = FReLU(out_channels)
        self.frelu2 = FReLU(out_channels)
        self.conv2 = conv3x3(out_channels, out_channels)
        self.bn2 = nn.BatchNorm2d(out_channels)

        # 入力と出力のチャンネル数が異なる場合、xをダウンサンプリングする。
        if in_channels != out_channels * self.expansion:
            self.shortcut = nn.Sequential(conv1x1(in_channels, out_channels * self.expansion, stride),#出力チャンネル数に合わせる
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, stride=1):
        ...
        if in_channels != out_channels * self.expansion:
            self.shortcut = nn.Sequential(conv1x1(in_channels, out_channels * self.expansion, stride),
                                          nn.BatchNorm2d(out_channels * self.expansion))
        else:
            self.shortcut = nn.Sequential()
...
class ResNet(nn.Module):
    def __init__(self, block, layers, num_classes=1000):
        ...
        self.relu = nn.ReLU(inplace=True)
        self.frelu1 = FReLU(self.in_channels)
    ...
    def _make_layer(self, block, channels, blocks, stride):     #if Res18,34 block = BasicBlock, else Bottleneck, blocksは繰り返し回数
        layers = []
